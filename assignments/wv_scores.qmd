...
```{r}
wv_games_avg <- wv_games |> 
  group_by(year) |>
  summarize(average = mean(differential))
```
...
```{r}
ggplot() + 
  geom_bar(data=wv_games_avg, aes(x=year, weight=average)) +
  labs(
    title="West Virginia Point Differential Steadily Grows Higher", 
    subtitle = "Outside of a few dips (the largest being in 2020), the point differential in games played by\nWest Virginia grows higher every year.",
    x="Year", 
    y="Differential"
) + theme_minimal()
```
