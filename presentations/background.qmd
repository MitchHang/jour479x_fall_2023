...
wta_matches_2022_new_top20_winlose <- wta_matches_2022_new_top20_win |> left_join(wta_matches_2022_new_top20_lose) |> replace(is.na(wta_matches_2022_new_top20_winlose), 0)
...
ggplot() + 
  geom_dumbbell(
    ...
    colour_xend = "red") + 
  theme_minimal() + 
  labs(title="Swiatek's win-loss gap grows larger in 2023", subtitle="Despite the Kansas City chiefs having the highest number of expected points added (EPA) after eight weeks \nof in-season games, the Minnesota Vikings have the greatest overall increase in its EPA, with a 229% \nincrease from Week 3 to Week 8 (green = matches won, red = matches lost).", caption="Source: WTA | By Mitchell Hang")  + theme_minimal() + 
  theme(
    plot.title = element_text(size = 16, face = "bold"),
    axis.title = element_text(size = 8), 
    plot.subtitle = element_text(size=10), 
